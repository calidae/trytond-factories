...
class FoundStock(StockMove):
    class Params:
        ...
        def warehouse(stub):
            if warehouse_id := Transaction().context.get('warehouse'):
                return Pool().get('stock.location')(warehouse_id)

    from_location = factory_trytond.ModelData('stock', 'location_lost_found')

    @factory.lazy_attribute
    def to_location(stub):
        if warehouse := stub.warehouse:
            return warehouse.storage_location
